...
async def create_template(payload: LayoutTemplateCreate, _=Depends(get_current_user)):
...
@router.get("/{tpl_id}")
async def get_template(tpl_id: str, _=Depends(get_current_user)):
...
async def remove_template(tpl_id: str, _=Depends(get_current_user)):
